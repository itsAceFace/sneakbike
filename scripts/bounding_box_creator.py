...
class BoundingBox:
    ...
    def __init__(
        ...
        self.color = self.__hex_to_rgba(self.color_hex)
    ...
    def draw_bounding_box(self):
        M_left_right = (np.array([self.color for _ in range(self.bound_thickness * (self.height - (2 * self.bound_thickness)))])
                        .reshape((self.height - (2 * self.bound_thickness)), self.bound_thickness, 4))
        M_top_bottom = (np.array([self.color for _ in range(self.bound_thickness * (self.width - (2 * self.bound_thickness)))])
                        .reshape(self.bound_thickness, (self.width - (2 * self.bound_thickness)), 4))
        M_center = np.zeros((self.height - (2 * self.bound_thickness),
        ...
        if not self.outer_corner_ornament:
            ...
            M_corner = (np.array([self.color for _ in range(self.bound_thickness * self.bound_thickness)])
                        .reshape(self.bound_thickness, self.bound_thickness, 4))

        else:
            # Otherwise, put a transparency for a placeholder.
            M_corner = (np.array([[0, 0, 0, 0] for _ in range(self.bound_thickness * self.bound_thickness)])
                        .reshape(self.bound_thickness, self.bound_thickness, 4))
    ...
    def draw_bounding_box_ornaments(self):
        ...
        ornament = (np.array([self.color for _ in range(ornament_size**2)])
                    .reshape(ornament_size, ornament_size, 4))

        if self.outer_corner_ornament:
            self.M[ornament_size:(2 * ornament_size),
                   ornament_size:(2 * ornament_size)] = ornament  # UL

            self.M[ornament_size:(2 * ornament_size),
                   (-2 * ornament_size):(-1 * ornament_size)] = ornament   # UR

            self.M[(-2 * ornament_size):(-1 * ornament_size),  # LL
                   ornament_size:(2 * ornament_size)] = ornament

            self.M[(-2 * ornament_size):(-1 * ornament_size),  # LR
                   (-2 * ornament_size):(-1 * ornament_size)] = ornament

        if self.inner_corner_ornament:
            self.M[(2 * ornament_size):(3 * ornament_size),  # UL
                   (2 * ornament_size):(3 * ornament_size)] = ornament

            self.M[(2 * ornament_size):(3 * ornament_size),  # UR
                   (-3 * ornament_size):(-2 * ornament_size)] = ornament

            self.M[(-3 * ornament_size):(-2 * ornament_size),  # LL
                   (2 * ornament_size):(3 * ornament_size)] = ornament

            self.M[(-3 * ornament_size):(-2 * ornament_size),  # LR
                   (-3 * ornament_size):(-2 * ornament_size)] = ornament
    ...
    def draw_inner_bounding_box(self):
        ...
        self.M[self.bound_thickness:(-1*self.bound_thickness),
